...
public class Game extends Thread{
    ...
    public static void setVolume() { //Set volume of game
        ...
        public void mousePressed(MouseEvent e) {
            ...
            if(!start) { //Only check before start is pressed
                if((x>checkBoxes[0].x && x<checkBoxes[0].x+checkBoxes[0].width) && (y>checkBoxes[0].y + 30 && y<checkBoxes[0].y+checkBoxes[0].height + 30)){
                    if(help && !buttons[1].visible){ //For setting the keys to on or off depending on if the checkbox is checked
                        help = false;
                        checkBoxes[0].visible = true;
                        checkBoxes[1].visible = false;
                    }
                    else if(!help && !buttons[1].visible){
                        help = true;
    ...
    private static class MouseMotion implements MouseMotionListener {
        ...
        public void mouseDragged(MouseEvent e) {
            int x = e.getX();
            int y = e.getY();
            if(!start && !buttons[1].visible) { //Only move slider if not start
